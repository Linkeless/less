...
interface SubscriptionCardProps {
  ...
  loading: boolean;
  userInfo: UserInfoResponse | null; // Still needed for the token for getFilteredUrl
  t: TranslationValues;
  handleCopyUrl: (url: string) => Promise<void>; // copyToClipboard is in utils, but this wraps notification logic
  // getFilteredUrl and formatExpireDate removed from props
}

export default function SubscriptionCard({
  subscription,
  loading,
  userInfo, // Keep userInfo for token access
  t,
  handleCopyUrl,
  ...
  if (!subscription || !userInfo) { // userInfo is needed for token
    return (
      <div className="relative group">
        <div className="absolute -inset-0.5 bg-gradient-to-r from-indigo-500 to-purple-500 rounded-2xl blur opacity-25 group-hover:opacity-50 transition duration-1000 group-hover:duration-200"></div>
        <div className="relative flex h-full flex-col overflow-hidden rounded-2xl bg-white dark:bg-gray-800 p-8 sm:p-10 gap-6 shadow-sm ring-1 ring-gray-950/5 dark:ring-white/5">
  ...
  // Token is still needed from subscription data for getFilteredUrl
  const token = subscription.data.token; 

  // Use imported getFilteredUrl, it needs the token from subscription data
                    ...
                    {[
                      ...
                      'onClick' in item ? (
                        ...
                        <a
                          key={item.id}
                          href={item.href}
                          className={`flex items-center justify-center gap-1.5 px-2.5 py-2 text-sm font-medium ${['clash', 'surge', 'shadowrocket', 'surfboard', 'quantumult-x'].includes(item.id)
                              ? 'bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-400 hover:bg-gray-200 dark:hover:bg-gray-700'
                              : 'bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-400 hover:bg-gray-200 dark:hover:bg-gray-700'
                            } rounded-lg transition-all duration-200 active:scale-95 focus:outline-none focus:ring-2 focus:ring-offset-2 ${['clash', 'surge', 'shadowrocket', 'surfboard', 'quantumult-x'].includes(item.id)
