import { on } from "events";

export default {
  common: {
    dashboard: 'Dashboard',
    product: 'Product',
    orders: 'Orders',
    signOut: 'Sign out',
    loading: 'Loading...',
    notifications: 'View notifications',
    openMenu: 'Open main menu',
    telegram: 'Telegram',
    connected: 'Connected',
    failed: 'Failed to load user information',
    processing: 'Processing...',
    cancel: 'Cancel',
  },
  auth: {
    signIn: 'Sign in',
    register: 'Register',
    email: 'Email address',
    password: 'Password',
    forgotPassword: 'Forgot password?',
    signInButton: 'Sign in',
    createAccount: 'Create your account',
    notAMember: 'Not a member?',
    alreadyHaveAccount: 'Already have an account?',
  },
  dashboard: {
    userInfo: 'User Information',
    balance: 'Balance',
    commission: 'Commission',
    memberSince: 'Member Since',
    trafficStats: 'Traffic Statistics',
    subscription: {
      noActive: 'No active subscription',
      expires: 'Expires',
      renew: 'Renew',
      reset: 'Reset',
      trafficUsage: 'Traffic Usage',
      used: 'Used',
      transfer: {
        total: 'Total Traffic',
        used: 'Used Traffic',
        remaining: 'Remaining',
        percentUsed: '{percent}% Used',
      }
    },
    uuid: {
      title: 'UUID',
      show: 'Show',
      hide: 'Hide',
      reset: 'Reset UUID',
      confirmReset: 'Are you sure you want to reset your UUID? This action cannot be undone.',
      cancel: 'Cancel',
    },
    nodes: {
      title: 'Subscribe Methods',
      selectRegion: 'Select Regional Entrance',
      selectedCount: '{count} Regional Entrance Selected',
      selectHint: 'Select Entry Region, default to Shenzhen',
    },
    traffic: {
      loading: 'Loading subscription info...',
      loadFailed: 'Failed to load subscription data',
      noData: 'No traffic data available',
      copyUrl: 'Copy URL',
      copied: 'Copied to clipboard',
      download: 'Download',
      upload: 'Upload',
      date: 'Date',
      statistics: 'Traffic Statistics',
      tooltipTitle: 'Traffic Details',
      downloadSpeed: 'Download Speed',
      uploadSpeed: 'Upload Speed',
    },
    purchase: {
      needSubscription: 'Please purchase a subscription to access the service.',
      purchaseNow: 'Purchase Now',
    },
    user: {
      lastLoginTime: 'Last Login',
      registerTime: 'Register Time',
      telegramBind: 'Telegram Binding',
    },
    enableForwarding: 'Enable Forwarding',
  },
  product: {
    title: 'Product List',
    billing: {
      monthly: 'Monthly billing',
      annual: 'Annual billing',
      period: 'Billing Period',
      quarterly: 'Quarterly billing',
      semiAnnual: 'Semi-Annual billing',
      perMonth: '/mo',
      perQuarter: '/quarter',
      perSemiAnnual: '/semi-annual',
      perYear: '/year',
      oneTime: 'One-Time',
    },
    price: {
      monthlyPrice: 'Monthly Price',
      quarterlyPrice: 'Quarterly Price',
      semiAnnualPrice: 'Semi-Annual Price',
      yearlyPrice: 'Yearly Price',
      twoYearPrice: '2-Year Price',
      threeYearPrice: '3-Year Price',
      oneTimePrice: 'One-Time Price',
    },
    order: {
      details: 'Product Details',
      traffic: 'Traffic Package',
      now: 'Order Now',
      selectedPlan: 'Selected Plan',
      duration: 'Duration',
      unlimited: 'Unlimited',
      coupon: {
        title: 'Have a coupon?',
        placeholder: 'Enter coupon code',
        verify: 'Verify',
        invalid: 'Invalid coupon code',
        verifying: 'Verifying coupon...',
        success: 'Coupon applied successfully',
      },
      confirmChange: 'Note: Changing subscription will override your current subscription.',
      subscriptionChangeWarning: 'Changing your subscription will override your current plan. Are you sure you want to proceed?',
      viewPendingOrders: 'View Pending Orders',
    },
    payment: {
      methods: 'Payment Methods',
      summary: 'Order Summary',
      selectedPlan: 'Selected Plan',
      originalPrice: 'Original Price',
      discount: 'Discount',
      priceAfterDiscount: 'Price After Discount',
      totalPayment: 'Total Payment',
      payNow: 'Pay Now',
      processing: 'Processing...',
      fee: '+{percent}% fee',
      balance: 'Account Balance',
      orderNumber: 'Order Number',
      orderTime: 'Order Time',
      orderStatus: 'Order Status',
      paymentChannel: 'Payment Channel',
      handleFee: 'Handling Fee',
      supportPay: 'Supported Payment Methods',
      waitPay: 'Waiting for payment',
      paid: 'Paid',
      cancelled: 'Cancelled',
      expired: 'Expired',
    },
    auth: {
      required: 'Authentication required',
      pleaseLogin: 'Please login to access this page',
      login: 'Login',
    }
  },
  orders: {
    history: 'Order History',
    period: 'Period',
    amount: 'Amount (¥)',
    actions: 'Actions',
    noOrders: 'No orders found',
    cancel: 'Cancel',
    unknown: 'Unknown',
    viewDetails: 'View details',
    confirmCancel: 'Are you sure you want to cancel this order?',
    cancelSuccess: 'Order cancelled successfully',
    cancelFailed: 'Failed to cancel order',
    unpaid: 'Unpaid',
    paid: 'Paid',
    monthly: 'Monthly',
    quarterly: 'Quarterly',
    semiAnnual: 'Semi-Annual',
    annual: 'Annual',
    twoYear: '2-Year',
    threeYear: '3-Year',
    confirmCancellation: 'Confirm Cancellation',
    cancelConfirmText: 'Are you sure you want to cancel this order? This action cannot be undone.',
    confirm: 'Confirm',
    orderNumber: 'Order Number',
    status: 'Status',
    createdAt: 'Created At',
    noOrdersFound: 'No orders found',
  },
  invite: {
    title: 'Invite Friends',
    link: 'Invite Link',
    invitedCount: 'Invited',
    rewardTotal: 'Total Reward',
    myCodes: 'My Invite Codes',
    copy: 'Copy',
    copied: 'Copied',
    loading: 'Loading...',
    loadFailed: 'Failed to load',
  },
  forwardingRules: {
    title: 'Forwarding Rules',
    description: 'Manage your port forwarding rules',
    addRule: 'Add Rule',
    name: 'Name',
    deviceGroup: 'Entry',
    listenPort: 'Listen Port',
    destination: 'Destination',
    status: 'Status',
    actions: 'Actions',
    copy: 'Copy',
    edit: 'Edit',
    delete: 'Delete',
    copySuccess: 'Copied successfully',
    portRange: 'Port Range',
    randomPort: 'Random Port',
    showing: 'Showing',
    to: 'to',
    of: 'of',
    results: 'results',
    previous: 'Previous',
    next: 'Next',
    diagnose: 'Diagnose',
    diagnosisResult: 'Diagnosis Result',
    diagnosisFailed: 'Diagnosis failed:',
    diagnosisRequestFailed: 'Failed to request diagnosis',
    close: 'Close',
    destinationHelp: 'Enter one destination address per line (e.g., 1.2.3.4:5678). Empty lines will be ignored.',
    confirmDeleteTitle: 'Delete Rule?',
    confirmDeleteText: 'Are you sure you want to delete this rule? This action cannot be undone.',
    noPermission: 'No forwarding permission',
    noPermissionDesc: 'Your account does not have port forwarding permission. Please contact the administrator or purchase the appropriate plan.',
  },
} as const;
